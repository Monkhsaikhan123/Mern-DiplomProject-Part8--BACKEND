...
app.get('/payment', async(req,res)=>{
    try {
        ...
        console.log(email)
        const query = {email:email}
        const result = await Payment.find(query).exec()
        res.status(201).json(result)
    } catch (error) {
        res.status(500).json({message:error.message});
    }
...
app.get('/paidusers', async(req,res)=>{
...
app.patch('/check/:id', async(req,res)=>{
    ...
    const {status} = req.body;
    try {
        const updateStatus = await Payment.findByIdAndUpdate(payId, {status : "Checking"},{
            new:true, runValidators:true
        })
        if(!updateStatus){
            return res.status(404).json({message:"Order not found"})
...
app.patch('/deliver/:id', async(req,res)=>{
    ...
    const {status} = req.body;
    try {
        const updateStatus = await Payment.findByIdAndUpdate(payId, {status : "Delivering"},{
            new:true, runValidators:true
        })
        if(!updateStatus){
            return res.status(404).json({message:"Order not found"})
...
app.get('/stats', async(req,res)=>{
    try {
        ...
        const products2 = await Products.countDocuments();
        const products = await Products2.countDocuments();
...
app.get('/charts', async(req,res)=>{
    try {
        const result = await Payment.aggregate([
            ...
            {
                $group:{
                    _id: '$ProductItemDetails.category',
